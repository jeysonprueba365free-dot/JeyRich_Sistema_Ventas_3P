...
export class BaseDatosService {
  ...
  private crearAlmacenes(db: IDBDatabase): void {
    if (!db.objectStoreNames.contains(this.ALMACENES.USUARIOS)) {
      const s = db.createObjectStore(this.ALMACENES.USUARIOS, { keyPath: 'id', autoIncrement: true });
      s.createIndex('idx_nombreUsuario', 'nombreUsuario', { unique: true });
      s.createIndex('idx_correo',        'correo',        { unique: true });
      s.createIndex('idx_rol',           'rol',           { unique: false });
    }
    if (!db.objectStoreNames.contains(this.ALMACENES.PROVEEDORES)) {
      const s = db.createObjectStore(this.ALMACENES.PROVEEDORES, { keyPath: 'id', autoIncrement: true });
      s.createIndex('idx_ruc',    'ruc',    { unique: true });
      s.createIndex('idx_nombre', 'nombre', { unique: false });
    }
    if (!db.objectStoreNames.contains(this.ALMACENES.CATEGORIAS)) {
      const s = db.createObjectStore(this.ALMACENES.CATEGORIAS, { keyPath: 'id', autoIncrement: true });
      s.createIndex('idx_nombre', 'nombre', { unique: true });
    }
    if (!db.objectStoreNames.contains(this.ALMACENES.PRODUCTOS)) {
      const s = db.createObjectStore(this.ALMACENES.PRODUCTOS, { keyPath: 'id', autoIncrement: true });
      s.createIndex('idx_codigo',      'codigo',      { unique: true });
      s.createIndex('idx_nombre',      'nombre',      { unique: false });
      s.createIndex('idx_categoriaId', 'categoriaId', { unique: false });
      s.createIndex('idx_proveedorId', 'proveedorId', { unique: false });
    }
    if (!db.objectStoreNames.contains(this.ALMACENES.CLIENTES)) {
      const s = db.createObjectStore(this.ALMACENES.CLIENTES, { keyPath: 'id', autoIncrement: true });
      s.createIndex('idx_documento', 'documento', { unique: true });
      s.createIndex('idx_nombre',    'nombre',    { unique: false });
    }
    if (!db.objectStoreNames.contains(this.ALMACENES.VENTAS)) {
      const s = db.createObjectStore(this.ALMACENES.VENTAS, { keyPath: 'id', autoIncrement: true });
      s.createIndex('idx_numeroVenta', 'numeroVenta', { unique: true });
      s.createIndex('idx_clienteId',   'clienteId',   { unique: false });
      s.createIndex('idx_usuarioId',   'usuarioId',   { unique: false });
      s.createIndex('idx_estado',      'estado',      { unique: false });
      s.createIndex('idx_fechaVenta',  'fechaVenta',  { unique: false });
    }
    if (!db.objectStoreNames.contains(this.ALMACENES.MOVIMIENTOS)) {
      const s = db.createObjectStore(this.ALMACENES.MOVIMIENTOS, { keyPath: 'id', autoIncrement: true });
      s.createIndex('idx_productoId', 'productoId', { unique: false });
      s.createIndex('idx_tipo',       'tipo',       { unique: false });
  ...
  private insertarDatosIniciales(evento: IDBVersionChangeEvent): void {
    ...
    const usuariosIniciales: Usuario[] = [
      { nombreUsuario: 'admin',   contrasena: 'admin123',   nombreCompleto: 'Administrador General', correo: 'admin@jeyrich.com',   rol: 'administrador', activo: true, fechaCreacion: new Date() },
      { nombreUsuario: 'jeyson',  contrasena: 'jeyson123',  nombreCompleto: 'Jeyson',                correo: 'jeyson@jeyrich.com',  rol: 'administrador', activo: true, fechaCreacion: new Date() },
      { nombreUsuario: 'richard', contrasena: 'richard123', nombreCompleto: 'Richard',               correo: 'richard@jeyrich.com', rol: 'administrador', activo: true, fechaCreacion: new Date() },
      { nombreUsuario: 'sergio',  contrasena: 'sergio123',  nombreCompleto: 'Sergio',                correo: 'sergio@jeyrich.com',  rol: 'vendedor',      activo: true, fechaCreacion: new Date() },
    ];
  ...
  async actualizarStock(
    ...
    const producto = await this.obtenerPorId<Producto>(this.ALMACENES.PRODUCTOS, productoId);
    ...
    const movimiento: MovimientoInventario = {
      productoId, tipo, cantidad, motivo, usuarioId,
    ...
    ventas
      ...
      .forEach(v =>
        v.detalles.forEach(d => {
          ...
          mapaProductos.set(d.productoId, { nombre: actual.nombre, total: actual.total + d.cantidad });
        })
      );
    return Array.from(mapaProductos.entries())
      .map(([productoId, { nombre, total }]) => ({ productoId, nombre, totalVendido: total }))
